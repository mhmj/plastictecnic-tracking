    ...
    <div>
        <table id="dt-shipping" class="dt-shipping table table-bordered table-hover table-striped w-100">
            ...
            <tr v-for="Shipping in data">
                <td><a :href="'shipment-details/'+Shipping.id" style="text-decoration: underline; color: blue">{{Shipping.delivery_no}}</a></td>
                <td>{{Shipping.company_id.name}}</td>
                <td>{{Shipping.status}}</td>
                <td>{{Shipping.created_at}}</td>
                <td style="width: 15%; display:flex; justify-content: flex-end">
                    <a :href="'shipment-details/'+Shipping.id"  class="btn btn-sm btn-outline-primary mr-2">
                        <i class="fal fa-edit"></i>
                    </a>
                    <a href="javascript:void(0);" v-on:click="openDeleteRecord(Shipping)" class="btn btn-sm btn-outline-danger mr-2">
...
//                this.Packaging = id;
            ...
            openDeleteRecord(id){
                $("#delete-shipping-modal").modal('show');
                this.Shipping = id;
            }
        }
